...
console.log('%c bb_game.js \n--------------------', 
            'color: blue; background-color: white;');

// General game CONSTANTS
const BB_VELOCITYARRAY = [-10, -9.5,  -9, -8.5,  -8, -7.5,  -7, -6.5,  -6, 
...
var bb_ballGroup, bb_ball, bb_x, bb_y;
var bb_ratio;
...
function preload() {
  ...
  //bb_collect.setVolume(0.1); doesn't alter the volume level...
  //bb_collect.outputVolume(0.1); produces an error...
}
...
function setup() {
  ...
  html_volume('bb_collect');// Register slider event to alter volume
...
function bb_startPause() {
  console.log('%c bb_startPause(): html_gameActive= ' + html_gameActive +
...
function bb_createBallSprites() { 
  ...
  for (var i = 0; i < BB_LEVELARRAY[bb_level].numBalls; i++) {
    bb_x = random(BB_LEVELARRAY[bb_level].dia/2, 
                  width-BB_LEVELARRAY[bb_level].dia/2)
    bb_y = random(BB_LEVELARRAY[bb_level].dia/2, 
                  height-BB_LEVELARRAY[bb_level].dia/2)
           
    bb_ball = new Sprite(bb_x, bb_y, BB_LEVELARRAY[bb_level].dia, 'd');
    bb_ball.vel.x = random(BB_VELOCITYARRAY) * BB_LEVELARRAY[bb_level].velScale;
    bb_ball.vel.y = random(BB_VELOCITYARRAY) * BB_LEVELARRAY[bb_level].velScale;
    if (html_debugStatus == 'y') {
      bb_ball.debug = true;
    }
    //bb_ball.vel.x = 0;
...
function bb_createTerminators() { 
  ...
  for (var i = 0; i < BB_LEVELARRAY[bb_level].numTerminators; i++) {
    bb_x = random(BB_TERMINATORDIA, width-BB_TERMINATORDIA);
    bb_y = random(BB_TERMINATORDIA, height-BB_TERMINATORDIA);
           
    bb_terminator = new Sprite(bb_x, bb_y, BB_TERMINATORDIA, 'd');
    bb_terminator.vel.x = random(BB_VELOCITYARRAY) * BB_LEVELARRAY[bb_level].velScale;
    bb_terminator.vel.y = random(BB_VELOCITYARRAY) * BB_LEVELARRAY[bb_level].velScale;
    if (html_debugStatus == 'y') {
