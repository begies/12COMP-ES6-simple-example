...
const DB_VELOCITYARRAY = [-10, -9.5, -09, -8.5, -08, -7.5, -07, -6.5, -06, 
...
function setup() {
  ...
  html_volume(db_spaceSound);// Register slider event to alter volume
...
function db_createBacteriaSprites() { 
  ...
  for (var i = 0; i < db_levelArray[db_level].numBacteria; i++) {
    ...
    db_bacteria.vel.x = random(DB_VELOCITYARRAY) * db_levelArray[db_level].velScale;
    db_bacteria.vel.y = random(DB_VELOCITYARRAY) * db_levelArray[db_level].velScale;
    //db_bacteria.vel.x = 0;
    //db_bacteria.vel.y = 0;
