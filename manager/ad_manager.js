...
// Import all the methods you want to call from the firebase module      //<=MODULAR-API
import { getDatabase, ref, update, remove }                              //<=MODULAR-API
...
// Display BB (Bouncing Ball) admin screen
// Input:  n/a
// Return: n/a
/**************************************************************/
function ad_BB() {
...
// Display CA (Collect Aliens) admin screen
// Input:  n/a
// Return: n/a
/**************************************************************/
function ad_CA() {
  ...
  // ENSURE THE READ FUNCTION NAME & THE PATH NAME ARE CORRECT           //<=======
  fb_readAll(CA, null, ad_processCAReadAll);                             //<=======
}

/**************************************************************/
// ad_DB()
// Input event; called when admin's DB button clicked
// Display DB (Dodge Bacteria) admin screen
// Input:  n/a
// Return: n/a
/**************************************************************/
function ad_DB() {
  ...
  // ENSURE THE READ FUNCTION NAME & THE PATH NAME ARE CORRECT           //<=======
  fb_readAll(DB, null, ad_processDBReadAll);                             //<=======
}
...
/**************************************************************/
// ad_TG()
// Input event; called when admin's TG button clicked
// Display TG (Tank Game) admin screen
// Input:  n/a
// Return: n/a
/**************************************************************/
function ad_TG() {
  ...
  ad_alterClass('ad_btn', 'grey');
  document.getElementById("b_adTG").style.backgroundColor   = "cyan";
  // ENSURE THE READ FUNCTION NAME & THE PATH NAME ARE CORRECT           //<=======
  fb_readAll(TG, null, ad_processTGReadAll);                             //<=======
}
...
function ad_processUSERReadAll(_result, _path,  _snapshot, _save, _error) {
  ...
  // Update interface's readAll result                                   //Mr Bob
  sessionStorage.setItem('fb_readAllStatus', _result);                   //Mr Bob
  p_fbReadAll.textContent = _result;                                     //Mr Bob
...
/**************************************************************/
// ad_processCAReadAll(_result, _path,  _snapshot, _save, _error)
...
/**************************************************************/
//                 _procFunc(_result, _path, _snapshot, _save, _error)
function ad_processCAReadAll(_result, _path, _snapshot, _save, _error) {
  console.log('%c ad_processCAReadAll(): result= ' + _result, 
              'color: ' + AD_COL_C + '; background-color: ' + AD_COL_B + ';');
...
/**************************************************************/
// ad_processDBReadAll(_result, _path,  _snapshot, _save, _error)
...
/**************************************************************/
//                 _procFunc(_result, _path, _snapshot, _save, _error)
function ad_processDBReadAll(_result, _path, _snapshot, _save, _error) {
  console.log('%c ad_processDBReadAll(): result= ' + _result, 
              'color: ' + AD_COL_C + '; background-color: ' + AD_COL_B + ';');
...
/**************************************************************/
// ad_processTGReadAll(_result, _path,  _snapshot, _save, _error)
// Called by fb_readAll to handle result of read ALL TG records request.
// Save data & update display with record info
/// Input:  result('waiting...', 'OK', 'error'), path, 
//         snapshot, where to save it & error msg if any
//         NOTE: This is the raw data, EG snapshot, and
...
/**************************************************************/
//                 _procFunc(_result, _path, _snapshot, _save, _error)
function ad_processTGReadAll(_result, _path, _snapshot, _save, _error) {
  console.log('%c ad_processTGReadAll(): result= ' + _result, 
              'color: ' + AD_COL_C + '; background-color: ' + AD_COL_B + ';');
    ...
    if (_snapshot.val() != null) {
      _snapshot.forEach(function(childSnapshot) {
        ...
        ad_adminArray.push({     
          uid:      childKey,
          gameName: childData.gameName,
          ratio:    childData.ratio,
          wins:     childData.wins,
          losses:   childData.losses,
          hits:     childData.hits,
          lives:    childData.lives,
          rounds:   childData.rounds,
          time:     childData.time
        });
      });
    }
...
// Called by ad_user, ad_BB, ad_CA, etc
// Create table header
  ...
  function finishTdEdit(td, isOk, _path, _dbKey, _dbFieldName) {
    if (isOk) {
      ...
      if (rtn[0]) {        // User input validated ok?
        ...
        td.style.background = 'red';
        // Update interface's write result                                //Mr Bob
        sessionStorage.setItem('fb_writeRecStatus', 'error');             //Mr Bob
        p_fbWriteRec.textContent = 'error';                               //Mr Bob
...
function ad_dbUpdateRec(_path, _dbKey, _dbFieldName, _data, _td) {
  ...
  const db = getDatabase();                                               //<=MODULAR-API
  const dbRef = ref(db, _path + '/' + _dbKey);                            //<=MODULAR-API
  update(dbRef, _data).then(function() {                                  //<=MODULAR-API
  ...
  //dbRef.update(_data).then(function() {
      ...
      console.log('%c ad_dbUpdateRec(): Update succeeded for ' + 
                  ...
                  'color: ' + AD_COL_C + '; background-color: ' + AD_COL_B + ';');
      // Update interface's write result                                  //Mr Bob
      sessionStorage.setItem('fb_writeRecStatus', 'OK');                  //Mr Bob
      p_fbWriteRec.textContent = 'OK';                                    //Mr Bob
...
function ad_dbDelRec(_tableId, _row, _item, _path) {
  ...
  const db = getDatabase();                                               //<=MODULAR-API
  const dbRef = ref(db, _path + '/' + key);                               //<=MODULAR-API
  remove(dbRef).then(function() {                                         //<=MODULAR-API
...
/**************************************************************/
// EXPORT FUNCTIONS
// List all the functions called by code or html outside of this module  //<=MODULAR-API
/**************************************************************/
export { 
  ad_user, ad_BB, ad_CA, ad_DB, ad_SI, ad_TG                             //<=MODULAR-API
}                                                                        //<=MODULAR-API            
